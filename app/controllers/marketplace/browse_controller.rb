module Marketplace
  class BrowseController < ApplicationController
    def index
      @products = apply_filters_and_sort(Product.exclude_purchased(current_user)).page(params[:page]).per(140)
    end

    def popular
      @products = apply_filters_and_sort(Product.ordered_by_purchase_count).page(params[:page]).per(140)
    end

    def free
      @products = apply_filters_and_sort(Product.where("price_cents <= 0")).page(params[:page]).per(140)
    end
  ...
  def featured
    ...
    @products = apply_filters_and_sort(Product.where(featured: true)).page(params[:page]).per(140)
    @products
  end
