...
module ProductConcern
  ...
  def convert_to_js_tree_format(structure)
    structure.map do |key, value|
      if value.is_a?(Hash)
        ...
        { text: key, icon: "#{icon_file(key)}", type: 'file' }
      end
    end
  end
  ...
  def icon_file(file_name)
    file_ext = File.extname(file_name).remove!(".")
    file_types = self.file_types
    if file_ext == ""
      file_types.each do |file_type|
        if file_type[:fileNames] && file_type[:fileNames].include?(file_name)
          begin
            icon_path = ActionController::Base.helpers.asset_path("icons/#{file_type[:name]}.svg")
          rescue 
            icon_path = 'jstree-file'
          end
          return icon_path
        end
      end
      ...
      file_types.each do |file_type|
        if file_type[:fileExtensions] && file_type[:fileExtensions].include?(file_ext)
          begin
            icon_path = ActionController::Base.helpers.asset_path("icons/#{file_type[:name]}.svg")
          rescue 
            icon_path = 'jstree-file'
  ...
  def file_types 
    ...
    {
      name: 'yaml',
      fileExtensions: ['yaml','yml.dist', 'yaml.dist', 'YAML-tmLanguage'],
    },
    {
      name: 'xml',
      fileExtensions: [
