...
class ProductsController < ApplicationController
  ...
  def code
    ...
    @product = Product.includes(:languages, :categories).with_attached_covers.friendly.find(params[:id])
    @related_products = @product.related_products
  ...
  def add_file_or_folder
    @product = Product.friendly.find(params[:id])
    path = params[:path]
    name = path.present? ? File.basename(path) : ""
    path = path.present? && path.include?("/") ? File.dirname(path) : ""

    item_type = params[:type]
    begin
      case item_type
      when "file"
        add_file_to_zip(@product, path, name, "")
        render json: { success: true, message: "File created successfully." }, status: :ok
      when "folder"
        add_folder_to_zip(@product, path, name)
        render json: { success: true, message: "Folder created successfully." }, status: :ok
      else
        render json: { success: false, error: "Invalid type specified." }, status: :unprocessable_entity
      end
    rescue => e
